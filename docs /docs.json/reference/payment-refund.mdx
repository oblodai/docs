...
**URL:** `https://api.oblodai.com/v1/payment/refund` · **Аутентификация:** обязательна ·
...
**Идемпотентность:** заголовок `Idempotency-Key` или тройка `(платёж, адрес, сумма)`.
...
| `order_id` | string | нет\* | Ваша ссылка на заказ платежа. |
| `address` | string | нет\*\* | Адрес назначения возврата. **По умолчанию — `payer_address`**, адрес, с которого пришли деньги. |
| `network` | string | нет | Сеть. |
| `amount` | string | нет | Частичная сумма. По умолчанию — вся полученная. |
...
|---|---|---|
| `Idempotency-Key` | нет | Любое уникальное значение (≤255 символов), одинаковое во всех повторах одного возврата. Повтор вернёт тот же ответ и заголовок `Idempotent-Replayed: true`. → [Идемпотентность](/reference/basics-idempotency) |
