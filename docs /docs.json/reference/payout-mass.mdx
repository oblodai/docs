...
|---|---|---|---|
| `payouts` | array | да | Массив ≤100 элементов; поля каждого — как в [`POST /v1/payout`](/reference/payout-create). |
| `source` | string | нет | Метка происхождения, применяется ко всем элементам без своего `source`. |
...
- [`POST /v1/payout/batch`](/reference/payout-batch) — до 5000 выплат, основной путь для больших пачек.
- [`POST /v1/payout`](/reference/payout-create) · [Объект выплаты](/reference/payout-object)
