...
|---|---|---|---|
| `uuid` | string | нет\* | Идентификатор платежа в Oblodai. |
| `order_id` | string | нет\* | Ваша ссылка на заказ. |
| `email` | string | нет | Кому отправить. По умолчанию — `payer_email`, заданный у платежа. |
...
| Поле | Тип | Описание |
|---|---|---|
| `sent` | bool | Письмо принято в очередь отправки. |
| `email` | string | Фактический получатель. |
| `uuid` | string | Платёж, по которому отправлено письмо. |
