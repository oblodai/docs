---
title: "POST /v1/auto-withdraw/list · /set · /delete"
---

Автовывод. Как только депозит зачисляет мерчанта в настроенном активе, **чистая** сумма
...
|---|---|---|---|
| `currency` | string | да | Актив, для которого включается автовывод. |
| `network` | string | да | Сеть. |
| `address` | string | да | Адрес назначения. |
| `min` | string | нет | Порог **в единицах актива** (не minor). По умолчанию `0` — без порога. |
...
| Поле | Тип | Обяз. | Описание |
|---|---|---|---|
| `currency` | string | да | Актив, для которого выключается автовывод. |
...
- [`POST /v1/payout`](/reference/payout-create) · [Форматы сумм и денег](/reference/basics-money)
