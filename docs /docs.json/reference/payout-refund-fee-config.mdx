---
title: "POST /v1/payout/refund-fee-config/get · /set"
---
...
|---|---|---|---|
| `fee_on_customer` | bool | да | `true` — клиент получает **net** (комиссию платит клиент); `false` — мерчант платит комиссию, клиент получает **gross**. |
...
- [`POST /v1/payment/refund`](/reference/payment-refund) · [Кто платит сетевую комиссию выплаты](/reference/payout-fee-config)
