...
<Warning>
  **Сейчас в Go modules — v1.0.x. Версия v1.1.0 ещё НЕ опубликована** (ждёт деплоя обновлённого
  шлюза). Команда выше поставит **v1.0.x**.

  Эта страница описывает **v1.1.0**. В v1.0.x — другая семантика идемпотентности (см. «Ошибки и
  повторы») и **нет** групп `Batches` / `Links` / `Splits` и методов `*Batch` / `SendEmail`.
  Всё, помеченное «с v1.1.0», появится после публикации.
</Warning>
...
**Повторы настраивать не нужно — они включены по умолчанию** (до 4 попыток, с учётом `Retry-After`).
Пустое поле `Retry` (`nil`) означает «дефолтные повторы», как и в остальных четырёх SDK. Если повторы
нужно **отключить**, сделайте это явно:
...
_, err = client.Payments.SendEmail(ctx, uuid, "" /* orderID */, "buyer@example.com")
```
...
- ⚠️ Фиатная цена + **только** `network` без `to_currency` → `400 payment.to_currency_required`.
...
умолчанию**, настраивать ничего не нужно. Отключить: `Retry: oblodai.NoRetry()`.

**Повтор безопасен, но механизм зависит от версии:**

| | **v1.0.x** (сейчас в реестрах) | **v1.1.0** (не опубликована) |
|---|---|---|
...
- [Обзор SDK](/sdk/overview) · [Приём первого платежа](/guides/accept-first-payment)
