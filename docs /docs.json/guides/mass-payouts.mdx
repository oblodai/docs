...
| | [`POST /v1/payout/batch`](/reference/payment-batch) | [`POST /v1/payout/mass`](/reference/payout-mass) |
|---|---|---|
...
Для любого сколько‑нибудь серьёзного объёма используйте [`POST /v1/payout/batch`](/reference/payment-batch):
...
- **[Массовые операции](/guides/batch-operations)** — батчи платежей, возвратов и выплат (до 5000).
- [`POST /v1/payout/batch` и `/v1/batch/info`](/reference/payment-batch)
- [`POST /v1/payout/mass`](/reference/payout-mass) · [`POST /v1/payout`](/reference/payout-create)
- [Объект выплаты](/reference/payout-object) · [Первая выплата](/guides/first-payout)
- [Ограничение частоты](/reference/basics-ratelimit) — почему пачки по 100 упираются в лимит.
