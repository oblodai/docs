---
title: "POST /v1/payment/link · /list · /info · /toggle"
---
...
|---|---|---|---|
| `title` | string | нет | Заголовок на странице оплаты. |
| `description` | string | нет | Описание на странице оплаты. |
| `amount_mode` | string | да | Режим суммы: `fixed` \| `open` \| `range`. |
| `currency` | string | да | **Валюта цены** — фиат (`USD`, `EUR`, `RUB`, …) или монета. Список — `pricing_currencies` из [`GET /v1/currencies`](/reference/currencies). |
| `amount_fixed` | string | нет\* | Сумма — для `fixed`. Обязательна в этом режиме. |
| `amount_min` | string | нет | Нижняя граница: «пол» для `open` (необязателен) и минимум для `range` (обязателен). |
| `amount_max` | string | нет\* | Верхняя граница — для `range`. Обязательна в этом режиме. |
| `pinned_currency` | string | нет | **Валюта расчёта** (монета), закреплённая за ссылкой. Пусто — монету выбирает покупатель. |
| `pinned_network` | string | нет | Сеть расчёта, закреплённая за ссылкой. Пусто — сеть выбирает покупатель. |
| `expires_in` | int64 | нет | Срок жизни ссылки, секунд от момента создания. **`0` (по умолчанию) — ссылка бессрочная.** |

\* Обязательность зависит от `amount_mode` — см. таблицу режимов выше.
...
| Поле | Тип | Обяз. | Описание |
|---|---|---|---|
| `limit` | int | нет | Сколько ссылок вернуть. |
...
| Поле | Тип | Обяз. | Описание |
|---|---|---|---|
| `link_id` | string | да | Идентификатор ссылки. |
| `limit` | int | нет | Пагинация списка платежей. |
| `offset` | int | нет | Смещение. |
...
|---|---|---|---|
| `link_id` | string | да | Идентификатор ссылки. |
| `active` | bool | да | `false` — выключить, `true` — включить обратно. |
