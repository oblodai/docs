---
title: "GET /v1/link/{id} · POST /v1/link/{id}/checkout"
---
...
|---|---|---|---|
| `amount` | string | нет\* | Сумма, которую ввёл покупатель, в валюте цены ссылки. **Обязательна для `open` и `range`**; для `fixed` **игнорируется** (сумму задаёт ссылка). |
| `currency` | string | нет\*\* | **Валюта расчёта** — монета, которой платит покупатель. Нужна, только если ссылка не закрепила `pinned_currency`. |
| `network` | string | нет\*\* | Сеть расчёта. Нужна, только если ссылка не закрепила `pinned_network`. |
| `payer_email` | string | нет | Email покупателя. На него автоматически уйдёт **чек** после оплаты. |

\* Для `fixed` можно не передавать вовсе.
\*\* Если ссылка закрепила монету/сеть, переданные значения игнорируются — выигрывает закрепление.
...
const r = await fetch(
  "https://api.oblodai.com/v1/link/5d3f2a71-9c84-4b0e-8d17-3e6a2c9f1b40/checkout",
  {
    ...
    body: JSON.stringify({ amount: "10.00", currency: "USDT", network: "tron" }),
  },
);
...
- [`POST /v1/payment/link` · /list · /info · /toggle](/reference/payment-link) — управление ссылками.
- [`GET /v1/pay/{id}`](/reference/pay-get) · [`POST /v1/pay/{id}/select`](/reference/pay-select) — hosted‑страница оплаты.
- [Объект платежа](/reference/payment-object) · [`POST /v1/payment/send-email`](/reference/payment-send-email)
