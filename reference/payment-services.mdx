...
| `is_available` | bool | Есть живой watcher метода и сеть сконфигурирована. |
| `limit.min_amount` | string | Минимальная сумма (`"0"` = без нижнего лимита). |
| `limit.max_amount` | string | Максимум (пусто = без верхнего лимита). |
| `commission.fee_amount` | string | Фиксированная часть комиссии. |
| `commission.percent` | string | Процентная часть комиссии. |
