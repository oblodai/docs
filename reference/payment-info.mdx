...
|---|---|---|---|
| `uuid` | string | нет\* | Идентификатор счёта в Oblodai. |
...
| Поле | Тип | Описание |
|---|---|---|
| `refund_status` | string | `none` — ничего не возвращали; `partial` — вернули часть; `full` — вернули всё оплаченное. |
| `refunds` | array | Каждый возврат: `uuid`, `status`, `amount`, `address`, `txid`, `is_final`, `created_at`. |
