...
**URL:** `https://api.oblodai.com/v1/payment/batch` · **Аутентификация:** обязательна ·
...
**Идемпотентность:** заголовок `Idempotency-Key` (на весь батч) + `order_id` каждого элемента.
...
|---|---|---|---|
| `payments` | array | да | Массив от 1 до **5000** элементов. Поля каждого элемента — **ровно те же**, что у [`POST /v1/payment`](/reference/payment-create). |
...
| Поле | Тип | Описание |
|---|---|---|
| `batch_id` | string | Идентификатор батча. С ним идите в [`POST /v1/batch/info`](/reference/batch-info). |
| `kind` | string | Вид батча — здесь всегда `payment`. |
