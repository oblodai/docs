...
| Поле | Тип | Описание |
|---|---|---|
| `code` | string | Персональный реферальный код. |
| `link` | string | Готовая пригласительная ссылка `…/?ref=<code>`. |
| `tier_bps` | array&lt;int&gt; | Доля **нашей** комиссии рефереру по месяцам, в bps. По умолчанию `[3000, 2000, 1000]` = 30 % / 20 % / 10 % за 1‑й, 2‑й, 3‑й месяц, дальше 0. |
| `referred_count` | int | Сколько мерчантов приведено. |
| `earnings_by_asset` | object | Валюта → сумма в **минимальных единицах** (minor units), строкой. |
