---
title: "POST /v1/payment/discount/list · /set"
---
...
|---|---|---|---|
| `currency` | string | нет | Валюта. Пусто = глобальный дефолт для всех монет. |
| `network` | string | нет | Сеть. Пусто = любая сеть данной валюты. |
| `discount_percent` | int | да | Процент, −99…99. Плюс — скидка, минус — наценка. |
...
- [`POST /v1/payment`](/reference/payment-create) · [Допуск недоплаты](/reference/payment-accuracy)
