...
**URL:** `https://api.oblodai.com/v1/payout/batch` · **Аутентификация:** обязательна ·
...
**Идемпотентность:** заголовок `Idempotency-Key` (на весь батч) + `order_id` каждого элемента (обязателен).
...
|---|---|---|---|
| `payouts` | array | да | Массив от 1 до **5000** элементов. Поля каждого элемента — **ровно те же**, что у [`POST /v1/payout`](/reference/payout-create). |
...
| `kind` | string | Вид батча — здесь всегда `payout`. |
...
- [`POST /v1/payout`](/reference/payout-create) — поля элемента. · [Объект выплаты](/reference/payout-object)
- [`POST /v1/payout/mass`](/reference/payout-mass) — легаси‑путь на ≤100 элементов.
- [`POST /v1/payment/batch`](/reference/payment-batch) · [`POST /v1/refund/batch`](/reference/refund-batch)
