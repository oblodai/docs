...
**URL:** `https://api.oblodai.com/v1/wallet` · **Аутентификация:** обязательна ·
...
**Идемпотентность:** заголовок `Idempotency-Key` или тройка `(currency, network, order_id)`.

### Заголовки

| Заголовок | Обяз. | Описание |
|---|---|---|
| `Idempotency-Key` | нет | Любое уникальное значение (≤255 символов), одинаковое во всех повторах. Повтор вернёт тот же ответ и заголовок `Idempotent-Replayed: true`. → [Идемпотентность](/reference/basics-idempotency) |
...
|---|---|---|---|
| `currency` | string | да | Символ валюты приёма (`USDT`, `BTC`, `ETH`, …). |
| `network` | string | да | Сеть приёма (`tron`, `ethereum`, `bitcoin`, …). |
| `order_id` | string | нет | Ваш идентификатор клиента/заказа. Закрепляет отдельный постоянный адрес за клиентом. |
...
- [Объект кошелька](/reference/wallet-object) · [Статические кошельки](/guides/static-wallets)
- [`POST /v1/wallet/block`](/reference/wallet-block) · [`POST /v1/balance`](/reference/balance)
