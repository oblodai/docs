...
<Warning>
  **`is_subtract` не влияет на саму выплату.** Поле в теле `/v1/payout` принимается, но
  плательщик комиссии всегда берётся из fee‑config проекта. При этом предрасчёт
