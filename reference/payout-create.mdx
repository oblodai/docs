...
<ParamField body="is_subtract" type="bool">
  **Принимается, но на саму выплату сейчас не влияет.** Кто платит сетевую комиссию, задаётся не в запросе, а настройкой проекта — см. [fee-config](/reference/payout-fee-config). (Поле учитывается только предрасчётом `/v1/payout/calculate`, см. примечание ниже.)
</ParamField>
