...
<Note>
  **Ветвитесь по `status`, а не по тексту `error`.** Текст — это `message`, а не код ошибки: он
  предназначен человеку и может измениться. Единственное исключение — проверка на строку выше, если
...
- [`POST /v1/payment/batch` · `/v1/refund/batch` · `/v1/payout/batch`](/reference/payment-batch) ·
  [`POST /v1/batch/info`](/reference/batch-info)
- [Массовые выплаты](/guides/mass-payouts) — легаси `/v1/payout/mass` и когда он ещё уместен.
- [Ограничение частоты](/reference/basics-ratelimit) — почему батч решает проблему `429`.
- [Идемпотентность](/reference/basics-idempotency) · [Рецепт устойчивого клиента](/guides/resilient-client)
- [Первая выплата](/guides/first-payout) · [Возвраты платежей](/guides/refunds)
