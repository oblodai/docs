...
Чтобы передать заголовок, добавьте его в свою обёртку `call()` (базовая версия — в
[Быстром старте](/guides/signing-requests)):
...
def call_resilient(path, payload, max_attempts=6):
    ...
    return last.json() if last is not None else None
```
